import { reqRegisterTest, reqLoginTest, reqUpdateUser } from '../api/index'
...
export const register = (user) => {
    ...
    return async dispatch => {
        // 如果有等待，说明该函数应该是异步函数
        const response = await reqRegisterTest({ username, password, type })
...
export const login = (user) => {
    ...
    // async dispatch 不能写
    return async dispatch => {
        const response = await reqLoginTest(user)
        const result = response.data
        if (result.code === 0) {
